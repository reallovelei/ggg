...
var newCommand = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/reallovelei/ggg/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				// 确认版本是否正确
				release, _, err = client.Repositories.GetReleaseByTag(context.Background(), "reallovelei", "ggg", version)
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/reallovelei/ggg/releases")
					return nil
				}
			}
			if version == "" {
				release, _, err = client.Repositories.GetLatestRelease(context.Background(), "reallovelei", "ggg")
		...
		fmt.Println("ggg框架版本：", release.GetTagName())

		templateFolder := filepath.Join(currentPath, "template-ggg-"+version+"-"+cast.ToString(time.Now().Unix()))
		...
		// 获取folder下的 reallovelei-ggg-xxx相关解压目录
		...
		for _, fInfo := range fInfos {
			// 找到解压后的文件夹
			if fInfo.IsDir() && strings.Contains(fInfo.Name(), "reallovelei-ggg-") {
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/reallovelei/ggg"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/reallovelei/ggg "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/reallovelei/ggg/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/reallovelei/ggg/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
