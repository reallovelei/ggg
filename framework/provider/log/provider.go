...
// GGGLogServiceProvider 服务提供者
type GGGLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *GGGLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return service.NewGGGConsoleLog
		}
	...
	case "single":
		return service.NewGGGConsoleLog
	case "rotate":
		return service.NewGGGRotateLog
	case "console":
		return service.NewGGGConsoleLog
	case "custom":
		//        return service.NewGGGCustomLog
	default:
		return service.NewGGGConsoleLog
	}
	return service.NewGGGConsoleLog
...
// Boot 启动的时候注入
func (l *GGGLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *GGGLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *GGGLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *GGGLogServiceProvider) Name() string {
