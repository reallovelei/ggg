...
// GGGRotateLog 代表会进行切割的日志文件存储
type GGGRotateLog struct {
	GGGLog
...
// NewGGGRotateLog 实例化GGGRotateLog
func NewGGGRotateLog(params ...interface{}) (interface{}, error) {
	// 参数解析
	...
	folder := appService.LogPath()
	if configService.IsExist("log.path") {
		folder = configService.GetString("log.path")
	}
	// 如果folder不存在，则创建
